...
from verl.experimental.agent_loop.tool_parser import TOOL_PARSER_ERROR_FUNCTION_NAME
...
class TestCallToolErrorHandling(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_tool_parser_error_feedback(self):
        """Parser errors should be returned to the model without tool lookup."""
        message = "Tool call parse error: missing </tool_call>."
    ...
    async def test_model_supplied_tool_parser_error_name_is_unknown(self):
        """The parser-error sentinel should only work for internal parser calls."""
        tool_call = FakeFunctionCall(
            name=TOOL_PARSER_ERROR_FUNCTION_NAME,
            arguments=json.dumps({"message": "fake parser error"}),
        )
        response, reward, _ = await self.loop._call_tool(tool_call, {}, self.agent_data)
        assert reward == 0.0
        assert "Unknown function" in response.text
        assert TOOL_PARSER_ERROR_FUNCTION_NAME in response.text

    async def test_tool_parser_error_feedback_is_truncated(self):
        """Parser feedback should respect max_tool_response_length."""
        self.loop.max_tool_response_length = 8
        self.loop.tool_response_truncate_side = "left"
        tool_call = FakeFunctionCall(
            name=TOOL_PARSER_ERROR_FUNCTION_NAME,
            arguments=json.dumps({"message": "x" * 32}),
            internal=True,
        )
        response, reward, _ = await self.loop._call_tool(tool_call, {}, self.agent_data)
        assert reward is None
        assert response.text == "x" * 8 + "...(truncated)"
