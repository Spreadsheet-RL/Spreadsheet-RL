...
from verl.workers.reward_manager.utils import merge_sandbox_workspace_id
...
class BatchRewardManager(AbstractRewardManager):
    ...
    def verify(self, data):
        ...
        extras_raw = data.non_tensor_batch.get("extra_info", [{} for _ in range(len(data))])
        workspace_ids = data.non_tensor_batch.get("sandbox_workspace_id", [None for _ in range(len(data))])
        tool_extra_fields = data.non_tensor_batch.get("tool_extra_fields", [None for _ in range(len(data))])

        extras = []
        for i in range(len(data)):
            extra = merge_sandbox_workspace_id(
                extras_raw[i] if i < len(extras_raw) else {},
                sandbox_workspace_id=workspace_ids[i] if i < len(workspace_ids) else None,
                tool_extra_fields=tool_extra_fields[i] if i < len(tool_extra_fields) else None,
            )
            extra["rollout_reward_scores"] = rollout_reward_scores[i] if i < len(rollout_reward_scores) else {}
            extras.append(extra)
