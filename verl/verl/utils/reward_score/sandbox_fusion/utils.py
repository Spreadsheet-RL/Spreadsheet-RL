...
def call_sandbox_api(
    ...
    Args:
        ...
        max_retries: Maximum number of requests to submit on retryable HTTP errors.
    ...
    # Optional per-rollout filesystem isolation (used by Spreadsheet-RL workloads).
    # Only include these keys when explicitly requested to preserve compatibility with older sandbox servers that
    # may forbid unknown fields.
    workspace_paths = workspace_paths or []
    if (workspace_id is not None) != bool(workspace_paths):
        logger.warning(
            "Ignoring workspace isolation because workspace_id/workspace_paths mismatch",
            extra={"workspace_id": workspace_id, "workspace_paths_len": len(workspace_paths)},
        )
    elif workspace_id is not None and workspace_paths:
        payload_dict["workspace_id"] = workspace_id
        payload_dict["workspace_paths"] = workspace_paths
...
def _process_single_case(
    ...
    """Run a single sandbox execution and return its raw metadata.

    This is a shared helper used by both evaluation-style runners (multiple
    test cases) and tool-style invocations (single case). When workspace
    parameters are provided, they enable per-request filesystem isolation on
    the sandbox server.

    Args:
        case_index: Zero-based index of the test case.
        stdin_data: Test input (typically passed via stdin).
        expected_output: Expected output (used by callers to grade correctness).
        sandbox_fusion_url: SandboxFusion `/run_code` endpoint.
        generation: Code to execute.
        timeout: Total timeout in seconds (compile + run).
        memory_limit_mb: Memory limit in megabytes.
        language: Sandbox language/runtime identifier (e.g. "python").
        concurrent_semaphore: Optional semaphore used to limit concurrency.
        fn_name: Optional function name to call for function-style tasks.
        workspace_id: Optional workspace/session id for per-request data isolation.
        workspace_paths: Optional list of dataset-relative paths to copy into the
            workspace before execution.
        max_retries: Optional override for retryable sandbox API submissions.

    Returns:
        Tuple `(case_index, metadata)` where `metadata` includes the sandbox API
        response fields (stdout/stderr/status/etc) and any request errors.
    """
    api_response = None
    error_msg = None
...
"""
    ...
    if max_retries is not None:
        api_call_kwargs["max_retries"] = max_retries
    try:
        if concurrent_semaphore:
            ...
            with concurrent_semaphore:
                ...
                api_response, error_msg = call_sandbox_api(**api_call_kwargs)
