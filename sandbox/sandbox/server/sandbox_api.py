...
class RunCodeRequest(BaseModel):
    ...
    workspace_id: Optional[str] = Field(
        None,
        description=(
            "Optional workspace/session id. When provided with workspace_paths, the server will copy the requested "
            "dataset paths into a per-workspace area and expose them via `data/...` inside the sandbox. This enables "
            "per-rollout filesystem isolation for workloads like Spreadsheet-RL."
        ),
    )
    workspace_paths: List[str] = Field(
        [],
        description=(
            "Relative paths under SANDBOX_DATA_ROOT to copy into the workspace before execution. "
            "Example: ['excelforum/<forum>/<thread_id>']."
        ),
    )

    @field_validator("workspace_id")
    @classmethod
    def _normalize_workspace_id(cls, value: Optional[str]) -> Optional[str]:
        if value is None:
            return None
    ...
    def _validate_workspace_fields(self) -> "RunCodeRequest":
        workspace_id_present = self.workspace_id is not None
        workspace_paths_present = bool(self.workspace_paths)
